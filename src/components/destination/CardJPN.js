...
import { cardJPN } from "../../data";
import Card from "../Card";
import { IoLocationSharp } from "react-icons/io5";
import { RiMoneyDollarCircleFill } from "react-icons/ri";

export default function CardJPN() {
  return (
    <>
      <div className="title">
        <h1 className="my-4 flex items-center justify-center gap-1 text-[1.35rem] font-medium text-dark-scy md:mb-8 md:justify-start">
          <IoLocationSharp /> Other
        </h1>
        <div className="card-jpn flex flex-wrap justify-center gap-7 lg:justify-between lg:gap-5">
          {cardJPN.map(({ image, title, location, price, preview }, index) => {
            return (
              <Card
                key={index}
